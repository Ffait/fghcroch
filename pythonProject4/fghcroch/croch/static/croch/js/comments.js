...
const commentProductId = commentForm.getAttribute('data-product-id');
...
async function createComment(event) {
    ...
    try {
        const response = await fetch(`/product/${commentProductId}/comments/create/`, {
            method: 'POST',
            headers: {
                                ...
                                <li class="card border-0">
                                    <div class="row color_back">
                                        ...
                                        <div class="col-md-10">
                                            <div class="card-body">
                                                <h6 class="card-title">
                                                    <a class="nav-link" href="${comment.get_absolute_url}">${comment.author}</a>
                                                </h6>
                                                <p class="card-text">
                                                    ${comment.content}
                                                </p>
                                                <a class="btn btn-sm btn-light btn-reply" href="#commentForm" data-comment-id="${comment.id}" data-comment-username="${comment.author}">Ответить</a>
                                                <hr/>
